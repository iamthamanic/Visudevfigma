...
**Standard (lokal):** `npm run dev` — lokaler Supabase-Stack (Docker) + Edge Functions + Vite + Preview-Runner. Siehe **`docs/HYBRID_DEV.md`**.

**Cloud (optional):** `npm run dev:cloud` — Projekt `tzfxbgxnjkthxwvoeyse`, **`VITE_SUPABASE_ANON_KEY` in `.env.local`** (siehe `.env.cloud.example`).
...
2. **`npm run dev:cloud`** – App spricht mit Auth, DB und Edge Functions in der Cloud.
...
Für lokales Supabase: `npm run dev` (Default, Docker) — siehe `docs/HYBRID_DEV.md`.
