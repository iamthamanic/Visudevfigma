...
**Standard:** Die App nutzt **Supabase Cloud** (Projekt `tzfxbgxnjkthxwvoeyse`). **`VITE_SUPABASE_ANON_KEY` in `.env.local` ist Pflicht** (siehe `.env.cloud.example`). Dann `npm run dev` starten.

**Hybrid (empfohlen für tägliche Entwicklung):** `npm run dev:hybrid` — lokaler Supabase-Stack + Edge Functions + Vite + Preview-Runner. Siehe **`docs/HYBRID_DEV.md`**.
...
Ohne lokale Supabase-URL nutzt die Vite-App die Cloud:

1. **`.env.local` anlegen** – `cp .env.cloud.example .env.local` und `VITE_SUPABASE_ANON_KEY` aus dem Dashboard eintragen (Settings → API → anon public).
...
Für lokales Supabase: `npm run dev:hybrid` oder `.env.local` mit `VITE_SUPABASE_URL=http://127.0.0.1:54321` (siehe `.env.example`).
...
**Zurück zu Cloud:** `VITE_SUPABASE_URL` aus `.env.local` entfernen; `VITE_SUPABASE_ANON_KEY` für Cloud beibehalten.
