...
export class GitHubAuthService extends BaseService {
  ...
  public async createAuthorizeUrl(returnUrl?: string): Promise<GitHubAuthorizeResponseDto> {
    if (!this.config.githubClientId) {
      throw new ConfigException("GitHub OAuth not configured. Please set GITHUB_CLIENT_ID.");
    }
    ...
    if (returnUrl) {
      await this.repository.setValue(`github_oauth_return:${state}`, {
        return_url: returnUrl,
        created_at: new Date().toISOString(),
      } satisfies GitHubAuthorizeReturnDto);
    }

    const authUrl = `${this.config.githubOAuthBaseUrl}/login/oauth/authorize?client_id=${this.config.githubClientId}&redirect_uri=${encodeURIComponent(
  ...
  public async handleCallback(
    ...
    if (returnData?.return_url) {
      try {
        redirectUrl = this.appendStateToUrl(returnData.return_url, effectiveState);
      } catch (error) {
  ...
  public async getSession(state: string): Promise<GitHubSessionDto> {
    const session = await this.repository.getValue<GitHubSessionDto>(`github_session:${state}`);
  ...
  public async getRepos(accessToken: string): Promise<GitHubRepoDto[]> {
    ...
    if (!response.ok) {
      throw new ExternalApiException(`GitHub API error: ${response.statusText}`, response.status);
    }
